...
const ROLE_RE =
  /\b(founder|co-?founder|ceo|cto|coo|cfo|cmo|cio|chief|partner|managing\s+partner|general\s+partner|\bgp\b|president|head\s+of|vp\b|vice\s+president|director|managing\s+director|owner|principal|manager|lead|engineer|investor|analyst|associate|officer|chair|advisor|operator|board)\b/i;

const NOISE_NAME_RE =
  /^(our team|the team|meet (the|our)|leadership|join us|read more|learn more|get started|sign up|subscribe|contact us|about us|privacy|careers|open roles|view all|see more|click here|home|menu|blog|news|updates)$/i;
...
export function looksLikePersonName(name) {
  ...
  if (NOISE_NAME_RE.test(clean)) return false;
  if (/https?:|@|www\.|\.com|\.io|\.ai\b/i.test(clean)) return false;
  if (/^(the|our|meet)\s/i.test(clean)) return false;
  const parts = clean.split(/\s+/);
  if (parts.length < 2 || parts.length > 5) return false;
  if (!parts.every((p) => /^[\p{L}][\p{L}'’.\-]*$/u.test(p))) return false;
  return true;
}

export function isPersonalLinkedIn(url) {
  return /linkedin\.com\/in\//i.test(String(url || ''));
}

export function isCompanyLinkedIn(url) {
...
export function socialHandle(url) {
  try {
    const u = new URL(url);
    return u.pathname.replace(/\/+$/, '').split('/').filter(Boolean).pop() || '';
  } catch {
    return '';
  }
}
...
/**
 * A roster row is only kept if it looks like a real person with a role or personal profile.
 */
export function isCredibleTeamMember(person, { companyName = '', companySocial = {} } = {}) {
  if (!person) return false;
  const name = String(person.name || '').replace(/\s+/g, ' ').trim();
  if (!looksLikePersonName(name)) return false;
  if (companyName && name.toLowerCase() === String(companyName).trim().toLowerCase()) return false;

  const linkedin = isCompanyLinkedIn(person.linkedin_url) ? null : person.linkedin_url;
  const twitter = person.twitter_url || null;
  const companyTw = socialHandle(companySocial.twitter || companySocial.twitter_url || '');
  const personTw = socialHandle(twitter);
  const twitterIsCompany = companyTw && personTw && companyTw.toLowerCase() === personTw.toLowerCase();

  const hasRole = ROLE_RE.test(String(person.title || ''));
  const hasPersonalLi = isPersonalLinkedIn(linkedin);
  const hasPersonalTw = Boolean(twitter) && !twitterIsCompany;

  return hasRole || hasPersonalLi || (hasPersonalTw && hasRole);
}

export function sanitizeTeamMember(person, extras = {}) {
  if (!isCredibleTeamMember(person, extras)) return null;
  const linkedin = isCompanyLinkedIn(person.linkedin_url) ? null : person.linkedin_url || null;
  return {
    ...person,
    name: String(person.name).replace(/\s+/g, ' ').trim(),
    title: person.title ? String(person.title).replace(/\s+/g, ' ').trim().slice(0, 120) : null,
    linkedin_url: isPersonalLinkedIn(linkedin) ? linkedin : null,
 ...
 */
export function preferLeadershipTeam(people, { max = 12, companyName = '', companySocial = {} } = {}) {
  const list = (Array.isArray(people) ? people : [])
    .map((p) => sanitizeTeamMember(p, { companyName, companySocial }))
    .filter(Boolean);
  if (list.length <= max) {
