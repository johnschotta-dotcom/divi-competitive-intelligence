...
import { alignedOverlap } from './overlap.js';
...
export function buildExportModel({
  ...
  const aligned = alignedOverlap(overlapScore, competitor?.true_competitor_label || comparison?.true_competitor_label, {
    isReference: isDivi,
  });
  const overlap = aligned.score != null ? String(aligned.score) : overlapScore != null ? String(overlapScore) : '—';
  const trueLabel = (aligned.label || competitor?.tier || '').toString().replace(/_/g, ' ');
