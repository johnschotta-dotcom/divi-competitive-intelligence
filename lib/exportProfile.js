...
import { formatCompetitorLabel, formatOverlapLevel, resolveOverlap } from './overlap.js';
...
export function buildExportModel({
  ...
  const aligned = resolveOverlap({
    level: comparison?.overlap_level || competitor?.overlap_level,
    score:
      competitor?.market_overlap_score ?? comparison?.market_overlap_score ?? competitor?.threat_score,
    label: competitor?.true_competitor_label || comparison?.true_competitor_label,
    isReference: isDivi,
  });
  const overlap = formatOverlapLevel(aligned.level);
  const trueLabel = formatCompetitorLabel(aligned.label || competitor?.tier);
...
export async function downloadProfilePdf(model) {
  ...
  y = wrapPdfText(
    doc,
    `${model.website || ''}  ·  Overlap ${model.overlap}  ·  ${model.trueLabel}  ·  ${model.analyzedAt}`,
    margin,
    y,
    maxWidth
  );
...
export async function downloadProfileDocx(model) {
  ...
  const children = [
    ...
    p(model.subtitle),
    p(
      `${model.website || ''} · Overlap ${model.overlap} · ${model.trueLabel} · Analyzed ${model.analyzedAt}`
    ),
  ];
