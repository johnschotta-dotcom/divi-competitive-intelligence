...
function referToCompetitors(text, { plural = true } = {}) {
  const noun = plural ? 'competitors' : 'this competitor';
  const Noun = plural ? 'Competitors' : 'This competitor';
  const poss = plural ? "competitors'" : "this competitor's";
  const Poss = plural ? "Competitors'" : "This competitor's";

  let next = String(text || '').replace(/\s+/g, ' ').trim().replace(/^[-•\s]+/, '');
  next = next.replace(/\bthis company\b/gi, noun);
  next = next.replace(/\bthese companies\b/gi, noun);

  next = next.replace(/^(They|Them)\b/, Noun);
  next = next.replace(/^Their\b/, Poss);
  next = next.replace(/([.!?]\s+)(They|Them)\b/g, `$1${Noun}`);
  next = next.replace(/([.!?]\s+)Their\b/g, `$1${Poss}`);

  next = next.replace(
    /\bthey (have|has|offer|offers|lead|leads|lack|lacks|position|positions|claim|claims|win|focus|serve|sell|provide|build|use|are|is|do not|don't)\b/gi,
    `${noun} $1`
  );
  next = next.replace(
    /\btheir (product|platform|site|website|marketplace|pricing|network|brand|focus|positioning|tools|workflow|deal flow|strength|edge)\b/gi,
    `${poss} $1`
  );

  next = next.replace(/\bcompetitors they\b/gi, noun);
  next = next.replace(/\bthey they\b/gi, noun);
  next = next.replace(/\bcompetitors\b/gi, noun);
  next = next.replace(/\bthis competitor\b/gi, noun);

  if (plural) {
    next = next.replace(/\bcompetitors (is|was|has|does)\b/gi, (_, verb) => {
      const map = { is: 'are', was: 'were', has: 'have', does: 'do' };
      return `competitors ${map[String(verb).toLowerCase()]}`;
    });
  }

  next = next.replace(/^competitors\b/, Noun);
  next = next.replace(/^this competitor\b/, Noun);
  next = next.replace(/^competitors'\b/, Poss);
  next = next.replace(/^this competitor's\b/, Poss);
...
function cleanInsight(text, companyName) {
  ...
  if (companyName) {
    ...
    next = next.replace(new RegExp(`\\b${escaped}\\b`, 'gi'), 'competitors');
  }
  return referToCompetitors(next, { plural: true });
}
...
function toInsight(partial) {
  ...
  return {
    id: partial.id,
    title: referToCompetitors(partial.title, { plural }),
    summary: referToCompetitors(partial.summary, { plural }),
    extras: (partial.extras || []).map((extra) => referToCompetitors(extra, { plural })),
    sources,
