...
async function claudeText(prompt, maxTokens = 4500) {
...
async function analyzeFromWebPresence(comp, competitorPresence, diviPresence) {
  ...
  const prompt = `You are Divi's competitive positioning analyst.

GOAL OF THIS TOOL:
Help Divi understand the market by reading what companies say on their websites and LinkedIn — who is truly trying to do the same job as Divi, where Divi wins, where Divi falls short, where messaging is the same, and how competitors differentiate.

EVIDENCE RULES (strict):
- Use ONLY the Divi web/LinkedIn corpus and the competitor web/LinkedIn corpus below.
- You may use ${diviGoldStandardBrief()} only as Divi's internal product framing when the live Divi website text is thin — still prefer live Divi website wording when present.
- Do NOT invent funding rounds, revenue, headcount, or press that is not stated on these pages.
- If something is not on the pages, say "Not stated on website/LinkedIn".
- Quote or closely paraphrase website language when making claims.
- TEAM / PEOPLE: If an EXTRACTED TEAM ROSTER block or /team /about page lists people, you MUST copy ALL of them into "founders" (use that field for the full team — partners, GPs, operators, not only company founders). Include title, LinkedIn, X, and bio when present. Do not drop people who are on the team page.

${diviGoldStandardBrief()}

=== DIVI WEB + LINKEDIN CORPUS ===
Website: ${DIVI_BASELINE.website}
LinkedIn: ${diviPresence.linkedin?.url || 'not found'}
${diviPresence.presenceText || '(Divi corpus unavailable)'}

=== COMPETITOR WEB + LINKEDIN CORPUS ===
Name: ${comp.name}
Website: ${comp.website}
LinkedIn: ${social.linkedin || competitorPresence.linkedin?.url || 'not found'}
Pages fetched from their site (crawled from homepage nav + about/blog/updates links):
${(competitorPresence.pagesFetched || []).map((u, i) => `${i + 1}. ${u}`).join('\n') || 'none'}
Top discovered links (scored): ${(competitorPresence.discoveredLinks || [])
    .slice(0, 12)
    .map((l) => l.url)
    .join(', ') || 'none'}
${competitorPresence.presenceText || '(competitor corpus unavailable)'}

Detected HTML tech signals (optional): ${detectedTech.map((t) => t.technology).join(', ') || 'none'}

Return ONLY valid JSON:
{
  "summary": "4-6 sentences: what their website says they do, who for, and how that overlaps Divi's stated job-to-be-done",
  "tagline": "from their site if possible",
  "target_audience": "who the website says they serve",
  "primary_value_prop": "their core claim in one sentence",
  "business_model": "pricing/model ONLY if stated on site, else Not stated on website/LinkedIn",
  "company_history_summary": "only what about/history pages say, else Not stated on website/LinkedIn",
  "founded_year": null,
  "headquarters": "only if stated else Unknown",
  "employee_estimate": "only if stated on LinkedIn/site else Unknown",
  "revenue_estimate": "Not stated on website/LinkedIn",
  "total_funding_display": "Not stated on website/LinkedIn",
  "history": [{"event_date": "", "title": "", "description": "must be from site", "event_type": "product"}],
  "founders": [
    {"name": "Full Name from team page", "title": "role from site", "linkedin_url": "https://linkedin.com/in/...", "twitter_url": null, "bio": "bio from site"}
  ],
  "funding_rounds": [],
  "funding_risk": 50,
  "team_risk": 50,
  ...
  "market_risk": 50,
  "growth_risk": 50,
  "funding_notes": "based on site silence or claims",
  "team_notes": "based on site/LinkedIn",
  "feature_notes": "overlap with Divi pillars from websites",
  "market_notes": "ICP overlap from websites",
  "growth_notes": "based on site/LinkedIn only",
  "strengths": [{"title": "what their website sells well", "why": "evidence from their pages"}],
  "weaknesses": [{"title": "gap vs what Divi's website claims", "divi_advantage": "how Divi's site positioning wins"}],
  "sentiment_score": 50,
  "sentiment_summary": "tone/confidence of their website+LinkedIn messaging (not social listening)",
  "positive_themes": "messaging themes from their site",
  "negative_themes": "gaps or weak claims vs Divi site",
  "media": [],
  "tech_stack": [{"technology": "", "category": "", "confidence": "high|medium|low", "evidence": "from HTML or site mention"}],
  "comparison": {
    "overall_verdict": "5-8 sentences: are they a true competitor to Divi's job-to-be-done? what market are they in? how should Divi think about them?",
    "where_we_win": ["Divi website/LinkedIn advantages vs them"],
    "where_we_fall_short": ["places their website/LinkedIn looks stronger or clearer than Divi's"],
    "where_same": ["shared claims / same market language / same jobs"],
    "where_they_differentiate": ["how they position differently from Divi"],
    "market_overlap_score": 0,
    "true_competitor_label": "direct|adjacent|tangential|not_a_competitor",
    "strategic_recommendation": "what Divi should do in product or messaging based on this website comparison",
    "evidence_basis": "website + LinkedIn only",
    "feature_matrix": [
      {
        "feature": "portfolio_tracking",
        "label": "Portfolio / cap table tracking",
        "divi": "what Divi site claims",
        "competitor": "what their site claims or Not stated",
        "winner": "divi|competitor|tie",
        "evidence": "short cite"
      }
    ]
  },
...
market_overlap_score guide (job-to-be-done overlap with Divi):
- 80-100 direct: same buyer + same core jobs (angel portfolio/syndicate/investor intelligence)
- 50-79 adjacent: overlapping buyers or partial job overlap
- 20-49 tangential: nearby market, weak overlap
- 0-19 not really a competitor

feature_matrix MUST include all keys: ${DIVI_BASELINE.comparisonDimensions.map((d) => d.key).join(', ')}`;

  let parsed = null;
  try {
    parsed = parseJsonFromModel(await claudeText(prompt, 4500));
  } catch (err) {
    console.error(`Analysis failed for ${comp.name}:`, err.message);
  }
  ...
  if (!parsed) {
    ...
    const fallback = ensureComparisonShape({
      summary: [
        facts.h1 ? `${comp.name} leads with: “${facts.h1}”.` : `${comp.name}: website crawled with limited structured analysis.`,
      ...
      target_audience: 'See website positioning vs Divi',
      ...
      strengths: facts.navOffers?.slice(0, 3).map((n) => ({
        title: `Site highlights: ${n.label}`,
        why: `Linked from their website (${n.url})`,
      })) || [{ title: 'Has a web presence', why: 'Site was reachable' }],
      weaknesses: [
        {
          title: 'Thin automated read of deeper product claims',
          divi_advantage: 'Divi can out-clarify AI angel OS positioning on divi.fund',
        },
      ],
      ...
      comparison: {
        overall_verdict:
          'Claude structuring failed or timed out; showing crawl-only website facts. Re-run analysis.',
        where_we_win: ['Divi gold-standard pillars remain the reference'],
        where_we_fall_short: ['Cannot fully score until Claude pass succeeds'],
        where_same: ['Both have public websites in/near investor tooling'],
        where_they_differentiate: ['Unknown until full analysis succeeds'],
        market_overlap_score: 40,
        true_competitor_label: 'adjacent',
        strategic_recommendation: 'Re-analyze this competitor alone.',
        feature_matrix: [],
      },
  ...
  // Always seed empty fields from deterministic website facts so UI isn't blank
  const facts = competitorPresence.websiteFacts || {};
    ...
    data.summary = [
      ...
      facts.people?.length
        ? `Team listed on site: ${facts.people
            ...
            .join(', ')}.`
        : null,
      facts.pagesFetched?.length
        ? `Pages we crawled: ${facts.pagesFetched.slice(0, 8).join(', ')}.`
        : null,
    ]
      .filter(Boolean)
      .join(' ');
  }
  if ((!data.tech_stack || !data.tech_stack.length) && facts.tech?.length) {
    data.tech_stack = facts.tech;
  }
  ...
  const allowRepair = process.env.ENABLE_REPAIR_PASS === '1';
  if (allowRepair && gaps.length) {
    try {
      const repairPrompt = `${prompt}

REPAIR PASS — previous JSON missed:
${gaps.map((g) => `- ${g}`).join('\n')}

Return a complete JSON object again. Keep evidence website/LinkedIn-only.`;
      const repaired = parseJsonFromModel(await claudeText(repairPrompt, 4500));
      if (repaired) {
        data = ensureComparisonShape({
          ...data,
          ...repaired,
      ...
      console.error(`Repair failed for ${comp.name}:`, err.message);
    }
  }

  data._completeness = scoreProfileCompleteness(data);

  // Merge deterministic website team extraction (Hustle Fund /team style) into founders
