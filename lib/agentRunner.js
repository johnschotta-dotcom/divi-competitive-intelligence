...
import { formatFounderForPrompt } from './peopleEnrich.js';
import {
  alignedOverlap,
  labelFromOverlapScore,
...
function ensureComparisonShape(data, { isReference = false } = {}) {
  ...
  const aligned = alignedOverlap(cmp.market_overlap_score, cmp.true_competitor_label, {
    isReference,
  });
...
Scores for market_overlap_score / true_competitor_label — these MUST match:
- 80-100 → true_competitor_label MUST be "direct": same primary buyer job as Divi — especially angel/individual-investor portfolio tracking, monitoring, and reporting. If that is their core product, score direct even when they lack AI, syndicates, or education. Missing pillars means Divi wins those matrix rows; it does NOT push them down to adjacent.
- 50-79 → true_competitor_label MUST be "adjacent": investor software with a different primary job (GP fund admin, LP portals, CRM-only, banking, back-office).
- 20-49 → true_competitor_label MUST be "tangential": shared audience only (deal marketplaces, content, communities) without portfolio-ops as the core offer.
- 0-19 → true_competitor_label MUST be "not_a_competitor": clearly outside angel/portfolio operating software — including companies you track for context only. Prefer not_a_competitor (score ≤19) over tangential when they are not a real alternative to Divi’s buyer job.
Never return a label that conflicts with the score band (e.g. 45 cannot be adjacent; not_a_competitor cannot be scored above 19).
Rule: judge overlap by primary job-to-be-done, not by how many Divi pillars they copy. One shared core job (portfolio ops for angels) is enough for 80+.
...
{"overall_verdict":"","where_we_win":["",""],"where_we_fall_short":[""],"where_same":[""],"where_they_differentiate":[""],"market_overlap_score":50,"true_competitor_label":"adjacent","strategic_recommendation":"","summary":"","tagline":"","target_audience":"","primary_value_prop":"","strengths":[{"title":"","why":""}],"weaknesses":[{"title":"","divi_advantage":""}],"feature_matrix":[]}`;
  ...
  // Merge deterministic team extraction (already enriched in fetchPresenceBundle)
  ...
  if (extracted.length) {
    ...
    for (const p of extracted) {
      ...
      byName.set(key, {
        ...
        prior_companies: p.prior_companies || prev.prior_companies || [],
        education: p.education || prev.education || [],
        location: p.location || prev.location || null,
        enrichment_source: p.enrichment_source || prev.enrichment_source || null,
      });
    }
...
function diviReferenceFromPresence(diviPresence) {
  return ensureComparisonShape({
    ...
    comparison: {
      overall_verdict:
        'Divi is the reference intelligence layer for private-market investors. Use competitor market_overlap_score and true_competitor_label to see who is actually chasing the same jobs (portfolio system of record + investor intelligence), not only shared audience.',
...
async function storeDeepProfile(comp, data, { isReference = false } = {}) {
  const aligned = alignedOverlap(
    data.comparison?.market_overlap_score,
    data.comparison?.true_competitor_label,
    { isReference }
  );
  if (data.comparison) {
  ...
  const label = isReference
    ? 'reference'
    : aligned.label || labelFromOverlapScore(overallRisk) || 'adjacent';
...
/** Heal score/label mismatches for all active competitors (score wins). */
export async function alignExistingOverlapLabels() {
  ...
  for (const row of rows || []) {
    ...
    const score = row.market_overlap_score ?? row.threat_score;
    const aligned = alignedOverlap(score, row.true_competitor_label);
    if (aligned.score == null || !aligned.label) continue;
  ...
  try {
    ...
    if (req.query?.alignOnly === '1' || req.query?.alignOnly === 'true') {
      return res.status(200).json({
        ...
        fixes: alignedRows,
        message: 'Overlap scores and labels realigned to scoring bands',
      });
    }
