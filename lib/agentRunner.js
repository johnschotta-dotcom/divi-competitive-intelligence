...
feature_matrix MUST include all keys: ${DIVI_BASELINE.comparisonDimensions.map((d) => d.key).join(', ')}`;
  ...
  if (!parsed) {
    return ensureComparisonShape({
      summary: `${comp.name}: website/LinkedIn corpus was insufficient to produce a full positioning read.`,
      tagline: comp.name,
      target_audience: 'Not stated on website/LinkedIn',
      primary_value_prop: 'Not stated on website/LinkedIn',
      business_model: 'Not stated on website/LinkedIn',
      company_history_summary: 'Not stated on website/LinkedIn',
      strengths: [{ title: 'Has a web presence', why: 'Site was reachable or partially reachable' }],
      weaknesses: [
        {
          title: 'Thin public website narrative vs Divi',
          divi_advantage: 'Divi can out-clarify AI angel OS positioning on divi.fund',
        },
      ],
      comparison: {
        overall_verdict: 'Insufficient website/LinkedIn evidence for a confident true-competitor call.',
        where_we_win: ['Clearer ability to state Divi gold-standard pillars when competitor site is thin'],
        where_we_fall_short: ['Cannot assess until competitor site content is richer'],
        where_same: ['Both operate on the public web in/near investor tooling'],
        where_they_differentiate: ['Unknown from available pages'],
        market_overlap_score: 30,
        true_competitor_label: 'tangential',
        strategic_recommendation: 'Re-run after confirming website URL; manually review their homepage.',
        feature_matrix: [],
      },
